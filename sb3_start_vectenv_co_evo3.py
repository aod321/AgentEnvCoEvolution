...
if __name__ == "__main__":
    ...
    # 参数定义
    gamepath = "/home/user/0815_newbuild_linux_maxsteps/0815_newbuild_linux_maxsteps.x86_64"
    ...
    # Tensroboard log
    tb_logs_path = f"./runs_3/{current_time}"
    ...
    try:
        ...
        for i in tqdm(range(TRAIN_EPOSIDES)):
            ...
            while mean_reward > REWARD_THREASHOLD:
                ...
                wave = vec_env.env_method("mutate_a_new_map", base_wave=base_wave)[0]
            ...
            if evolve_count > 0:
                ...
                vec_env.env_method(method_name="set_wave", wave=wave)
                vec_env.env_method(method_name="render_in_unity")
                ...
                print(f'Evaluation on all Done: Mean reward on all map: {mean_reward_1} +/- {std_reward_1:.2f}')
                if EXTRA_EVAL:
